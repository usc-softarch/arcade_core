...
public class ACDC {
  ...
  private static boolean matches(String s1, String s2) {
		...
		List<String> v = new ArrayList<>(s2.length()); 

		for(int i=0 ; i<s2.length() ; i++) v.add(Character.toString(s2.charAt(i)));
			
		for(int j=0; j<s1.length(); j++)
			containsNoOther = v.contains(Character.toString(s1.charAt(j)));
  ...
  public static void main(String [] args) {
    ...
    for(int u=0; u<args.length; u++)
    	if(args[u].equalsIgnoreCase("-h")) err_AND_exit(run_name);	
    ...
    outputName = args[1];
		...
		if (outputName.endsWith(".rsf")) output = new RSFOutput();
    ...
    Pattern inducer = null;
		...
		if (selectedPatterns == null) selectedPatterns = defaultPatterns;
		if (inducer == null) inducer = new DownInducer(root);
    ...
    //add new pattern here
		List<Pattern> vpatterns = new ArrayList<>(); //will contain the ordered patterns to be executed
    ...
    Iterator<Pattern> iv = vpatterns.iterator();
    while (iv.hasNext()) {
			Pattern p = iv.next();
